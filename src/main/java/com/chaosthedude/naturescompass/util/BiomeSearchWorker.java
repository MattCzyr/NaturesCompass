...
public class BiomeSearchWorker implements WorldWorkerManager.IWorker {
	...
	public BiomeSearchWorker(ServerLevel level, Player player, ItemStack stack, ResourceLocation biomeKey, BlockPos startPos, List<BlockPos> prevPos) {
		this.level = level;
		this.player = player;
		this.stack = stack;
	...
	private void checkConnections() {
		...
		for (int y : yValues) {
			...
			final Optional<ResourceLocation> checkBiomeKey = BiomeUtils.getKeyForBiome(level, biomeAtCheck);
			if (checkBiomeKey.isPresent() && checkBiomeKey.get().equals(biomeKey)) {
		...
		if (foundMatchingBiome) {
			...
			// If there's a stretch of consecutive non-matching samples equal to 25% the total
			// samples (max 10), consider the candidate not connected to the previous location
			int consecutiveNonMatchingThreshold = Mth.clamp(connectionsCheckSamples / 4, 1, 10);
	...
	private void succeed() {
		...
		if (!stack.isEmpty() && stack.getItem() == NaturesCompass.naturesCompass) {
			((NaturesCompassItem) stack.getItem()).succeed(stack, player, biomeKey, x, z, prevPos, samples, ConfigHandler.GENERAL.displayCoordinates.get());
		} else {
	...
	private void fail() {
		...
		if (!stack.isEmpty() && stack.getItem() == NaturesCompass.naturesCompass) {
			((NaturesCompassItem) stack.getItem()).fail(stack, player, biomeKey, roundRadius(getRadius(), 500), samples);
		} else {
			NaturesCompass.LOGGER.error("BiomeSearchWorker " + id + ": Invalid compass after search");
		}
		finished = true;
